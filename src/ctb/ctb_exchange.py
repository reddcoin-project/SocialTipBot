...
class CtbExchange(object):
    ...
    def get_ticker_value(self, _name1=None, _name2=None):
        ...
        if not self.supports_pair(_name1=_name1, _name2=_name2):
            raise Exception(
                "CtbExchange::get_ticker_value(%s, %s, %s): pair not supported" % (self.conf.domain, _name1, _name2))

        results = []
        for myurlpath in self.conf.urlpaths:
            for myjsonpath in self.conf.jsonpaths:

                toreplace = {'{THING_FROM}': _name1.upper() if self.conf.uppercase else _name1.lower(),
                             '{THING_TO}': _name2.upper() if self.conf.uppercase else _name2.lower()}
                for t in toreplace:
